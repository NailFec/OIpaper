...
#show: BUPTBachelorThesis.with(
    ...
    keywordsZH: ("信息学奥赛", "离线算法", "数据结构", "莫队算法", "分治思想", "分块", "树状数组", "线段树", "归并树", "扫描线")
)
...
首先，我们需要预处理 $a_i$ 在序列中上一次出现的位置 $bef_i$ 和下一次出现的位置 $nxt_i$。一种简单的方案可以随着依次读入序列 ${a_i}$ 的过程进行预处理。定义一个哈希表 $last_x$，表示值 $x = a_i$ 在当前读入的数列 $[a_1, a_i)$ 中，最后出现的位置。那么即可赋值 $bef_i <- last_x, space nxt_(last_x) <- i, space last_x = i$。最终随着序列 $a_i$ 的读入，所有数据的 $bef$ 与 $nxt$ 值均处理完成。当然，我们也可以在读完序列之后，利用哈希表 $last$ 对序列作正向和反向的遍历，分别计算 $bef$ 和 $nxt$ 数组。两种预处理方案的时间复杂度均为 $O(N)$。

然后，我们需要进行分块的预处理操作。先将序列分为若干个块。设序列长度为 $N$，令块的大小 $S = floor(sqrt(N))$，则一共会有 $D = ceil(N / S)$ 个块。对于每个块，暴力地计算块内不同元素的个数。此操作可以借用一个布尔数组 $exi_(a_i)$ 记录 $a_i$ 是否出现过。具体来说，若处理的块为 $[a_m, a_n]$，从 $a_m$ 开始遍历块内的每个元素。如果 $exi_(a_i) = 0$，则说明 $a_i$ 第一次在块内出现，则块内不同元素的个数 $cnt <- cnt + 1$；且令 $exi_(a_i) = 1$，标记值 $a_i$ 已在块内出现。另外，我们也可以借助 $bef$ 数组来计算。如果 $bef_i < m$，即上一个值 $a_i$ 出现在第 $m$ 个元素之前，也就是说从 $m$ 到 $i - 1$ 内都没有出现过值 $a_i$，$a_i$ 在块内第一次出现，令 $cnt <- cnt + 1$。

记二维数组 $f_(i,j)$ 表示第 $i$ 到第 $j$ 个块内不同元素的个数，则第 $i$ 个块内不同元素的个数即为 $f_(i, i)$。

我们可以使用递推的方法，计算 $f$ 数组的所有值。其递推公式为 $f_(i, j) = f_(i, j - 1) + Delta cnt$，其中 $Delta cnt$ 表示第 $j$ 个块内相对第 $i~j-1$ 个块新增的不同元素的个数，即仅在第 $j$ 个块内出现，而没有在第 $i$ 到第 $j - 1$ 个块内出现的元素的个数。此操作可利用 $bef$ 数组实现。对于每个递推过程，我们可以在 $O(S)$ 的时间复杂度内完成转移。至此，我们完成了所有预处理操作。
...
先进行离线操作，即对询问区间进行排序。莫队算法也使用了类似分块的思想，将区间分为多个块，所有左端点 $l$ 在同一个块内的区间为一个整体。对于每个块，按照区间的右端点从小到大进行排序。最终的结果如 @Gmo1 所示。这样离线便是莫队算法的核心，其好处在后文可以体现。

用两个指针 $p, q$ 表示现已计算好的区间 $[p, q]$，记该区间内不同元素的个数为 $cnt$，区间内数 $x$ 出现的次数为 $nsum_x$。在处理区间 $[l_i, r_i]$ 时，将左指针 $p$ 逐位移动到 $l_i$。如果 $p$ 需要向左移动 $1$ 位，且 $nsum_(p-1) = 0$，即 $a_(p-1)$ 在 $[p,q]$ 中没有出现过，则将 $cnt$ 增加 $1$；否则 $cnt$ 保持不变。同时，也要将 $nsum_a_(p-1)$ 增加 $1$，表示新的 $[p, q]$ 内该数的出现次数增加 $1$ 次。如果 $p$ 需要向右移动 $1$ 位，则将 $nsum_a_(p+1)$ 减少 $1$。如果现在 $nsum_a_(p+1) = 0$，即最后的一个 $a_(p+1)$ 被移出了 $[p, q]$，则将 $cnt$ 减少 $1$；否则 $cnt$ 保持不变。右端点 $q$ 的移动方式与左端点相反。

通过这样的转移方式，我们可以在上一个区间的答案的基础上计算得到下一个区间的答案。经过离线排序后的询问区间，如 @Gmo1 所示，我们不难发现：左指针在同一个块内的移动距离较小，而右指针在每个块内都是单调增加的。这样的离线使总转移距离尽可能小，这便是莫队算法的核心思想。
...
又因为 $max_(ceil(sqrt(N)) - 1)$ 最大为 $N$，因此 $l$ 的总时间复杂度最坏情况下为 $O(N sqrt(N))$。@Bmo
...
块的大小对莫队算法的时间复杂度影响非常显著。例如，$M$ 与 $sqrt(N)$ 同阶时，最优的块大小 $S = N / sqrt(M) = N^(1-1/2 times 1/2) = N^(3/4)$，时间复杂度可以达到 $O(N^(5/4))$。如果块的大小取 $sqrt(N)$，其时间复杂度只有 $O(N sqrt(N))$。 @Bmo
...
通过上一种优化方法，我们可以发现，$cnt$ 数组的访问和修改较为耗时。实际上，我们可以不记录 $cnt$ 数组。考虑在读入时预处理 $a_i$ 上一次出现的位置 $bef_i$ 和下一次出现的位置 $nxt_i$。在转移指针 $p, q$ 时，只需要访问 $bef_i$ 或 $nxt_i$ 中的一个元素便可知道数 $a_i$ 是否在原区间 $[p, q]$ 出现过，且无需进行对 $bef$ 或 $nxt$ 的修改操作。另外，在使用此方法优化时，离散化输入序列的值就没有必要了。使用该优化，在洛谷测试结果可以提高 $60$ 分，顺利通过该题，最大的数据甚至可以在 $1.01 s$ 内通过。 @Bmobetter2
...
在实际工程应用中，大规模数据查询问题广泛存在于数据库系统、搜索引擎、数据挖掘、流式计算等领域。现代数据库系统的索引设计和查询优化借鉴了分块和树状结构的思想，如 B+ 树索引和分区表技术；搜索引擎的倒排索引和文档检索系统利用了分块排序和树状结构进行高效检索；大数据处理框架如 Hadoop 和 Spark 在数据分片和聚合计算中应用了分块思想；时序数据库在处理时间序列数据时，利用区间树数据结构提升查询效率；实时计算系统处理流式数据时，利用滑动窗口结合高效的树形数据结构进行增量计算。这些实际应用证明了本文所讨论算法的现实价值，也启示我们在算法设计中要兼顾理论复杂度和工程实用性。
